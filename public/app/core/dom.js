...
export function wordWithAccentHtml(text, accentIndex) {
  ...
  // Переносы считаются на ПОЛНОМ исходном слове (со всеми слогами, включая
  // тот, что несёт ударную гласную), а не отдельно для текста до и после
  // неё: часть слоговых границ проходит ИМЕННО между соседним слогом и
  // слогом ударной буквы. Например, в "Серендипность" (ударная "и", индекс
  // 6) граница "Серен-дипность" — это стык слогов "рен" и "ди", а "и" здесь
  // и есть ударная буква; если считать только по "before" = "Серенд", эта
  // граница не видна вообще, и остаётся лишь самая ранняя точка переноса —
  // слово рвётся в первом попавшемся месте, даже когда ширины хватило бы
  // показать больше текста.
  //
  // Поэтому: считаем переносы на полном слове и только потом отбрасываем
  // те точки, что попали бы вплотную к самой ударной букве — такая пара
  // всё равно склеена в один nowrap-фрагмент чуть ниже, и переносить
  // непосредственно там нельзя (мягкий перенос иначе окажется внутри этого
  // фрагмента и собьёт разбор "буква до / ударная буква / буква после").
  ...
  // .accent-mark — inline-block (см. components.css: нужно для однозначного
  // containing block абсолютно спозиционированной засечки). Но граница
  // inline-block-элемента сама по себе — допустимая точка переноса строки
  // в браузерах, независимо от того, есть ли рядом пробел; юникодовский
  // WORD JOINER (\u2060) её не отменяет — это подтверждено рендером в
  // реальном Chromium. Поэтому вместо "символа, запрещающего перенос"
  // склеиваем span вместе с ОДНИМ символом до и ОДНИМ символом после в один
  // white-space: nowrap фрагмент (.accent-mark-unit, см. components.css) —
  // внутри него браузер физически не может разорвать строку. Благодаря
  // фильтру выше эти крайние символы гарантированно настоящие буквы, а не
  // случайно попавший сюда мягкий перенос.
...
// Кнопка отправки формы (вход/сохранить/изменить/зарегистрироваться и
// т.п.) тусклая и неактивная, пока хотя бы одно из перечисленных полей
// пустое, и становится обычной цветной кнопкой, как только все они
// заполнены — не нужно дублировать эту проверку в каждой странице с
// формой. Визуальный эффект "тусклая/неактивная" уже есть в CSS
// (.btn:disabled), здесь только логика включения/выключения атрибута.
//
// Проверяется именно "непустое значение" (после trim), а не встроенная
// HTML5-валидация (required, type="email" и т.п.) — она всё равно
// сработает при попытке отправить форму, а здесь нужен более простой и
// мгновенный визуальный сигнал "все поля тронуты".
//
// Вызывать после того, как инпуты и кнопка уже вставлены в DOM. Слушатель
// вешается на событие "input", так что состояние кнопки обновляется на
// каждое нажатие клавиши, а не только при потере фокуса.
export function wireRequiredFieldsToggle(button, inputs) {
  function update() {
...
// Русское склонение существительного по числу — обычное "N === 1 ? форма
// : форма" (как было раньше для "слово"/"слов") даёт неправильный текст
// почти для половины чисел: "2 слов", "21 слов", "3 слов" вместо "2 слова",
// "21 слово", "3 слова". Настоящее правило зависит от последней цифры И от
// последних двух цифр числа (11-14 — всегда "many", независимо от
// последней цифры).
