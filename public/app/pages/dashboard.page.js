...
export async function renderDashboard() {
  ...
  try {
    ...
    document.getElementById("logoutBtn").onclick = async () => {
      ...
      // аккаунт.
      //
      // Для гостя поэтому не выходим сразу, а показываем полноценный экран
      // подтверждения (см. confirmLogout.page.js) — обычную страницу SPA
      // без перезагрузки, а не window.confirm(). У обычного
      // зарегистрированного пользователя такого риска нет (данные и так
      // привязаны к его email), поэтому "Выйти" для него работает как
      // обычно — сразу.
      if (user.isGuest) {
        navigate("/leave");
        return;
